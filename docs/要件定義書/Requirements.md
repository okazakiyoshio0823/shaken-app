...
| EST-04 | 見積計算バリデーション | 必須 | 車検点検漏れ、および一般整備時の法定費用過剰入力の警告 |
| EST-05 | 値引き適用 | 任意 | 部品、鉱油類、工賃への値引き率設定 |
| EST-06 | 消費税計算 | 必須 | 税率10%での自動計算 |
| EST-07 | 法定費用の一括リセット・復元 | 任意 | クリアボタンによる0円化と、直後のみ使用可能な「元に戻す（Undo）」ボタンによる復元機能 |

### 5. 出力・共有

| 要件ID | 要件名 | 優先度 | 説明 |
|--------|--------|--------|------|
| OUT-01 | 印刷プレビュー | 必須 | A4形式でのプレビュー表示。複数ページ対応と要素被りの完全防止 |
| OUT-02 | PDF出力 | 必須 | 見積書/請求書/領収書のPDF生成。A4固定高を用いた白紙出力防止 |
